document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (updatedUser.stats && updatedUser.stats.length > 0) {
        ...
        if (dota2Item && dota2Stat) {
            dota2Item.textContent = `${dota2Stat.hours_played}H 22Mins`;
        }
        if (fortniteItem && fortniteStat) {
            fortniteItem.textContent = `${fortniteStat.hours_played}H 22Mins`;
        }
        if (csgoItem && csgoStat) {
            csgoItem.textContent = `${csgoStat.hours_played}H 46Mins`;
        }
    }
